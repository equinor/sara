...
SARA_CLIENT_SECRET=$(az ad app credential reset --id $CFG_SARA_CLIENT_ID --append --display-name $CFG_SARA_SECRET_NAME --query password --out tsv)
...
az keyvault secret set --vault-name $CFG_VAULT_NAME \
                       --name $CFG_SARA_SECRET_NAME \
                       --value $SARA_CLIENT_SECRET \
