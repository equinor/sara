...
def notify_fencilla_started(inspection_id: str, workflow_name: str):
    logger.info(
        f"Notify Fencilla started for inspection {inspection_id} and workflow {workflow_name}"
    )
...
def notify_fencilla_result(inspection_id: str, is_break: bool, confidence: float):
    logger.info(f"Notify Fencilla result for inspection {inspection_id}")
...
def notify_fencilla_exited(
    ...
    logger.info(
        f"Notify Fencilla exited for inspection {inspection_id} with status {workflow_status}"
    )
