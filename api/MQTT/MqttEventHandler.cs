...
namespace api.MQTT
{
    ...
    public class MqttEventHandler : EventHandlerBase
    {
        ...
        private bool ValidateIsarInspectionResultMessage(IsarInspectionResultMessage message)
        {
            ...
            bool isValid = Validator.TryValidateObject(message, new ValidationContext(message), validationResults, true);
            isValid &= Validator.TryValidateObject(message.InspectionDataPath, new ValidationContext(message.InspectionDataPath), validationResults, true);
            isValid &= Validator.TryValidateObject(message.InspectionMetadataPath, new ValidationContext(message.InspectionMetadataPath), validationResults, true);
            if (!isValid)
            {
